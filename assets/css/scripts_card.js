...
(function () {
    ...
    function renderLeftMenuFooter(site) {
        const container = $('diamondLeftMenuFooter');
        if (!container) return;
        const footer = site?.footer_meta?.menu_footer || {};
        const logo = site?.assets?.icons?.logo_png || site?.assets?.icons?.logo_ori_png || './assets/img/Emran_Ali_Logo_Fav.png';
        const copyrightYear = typeof SiteUtil !== 'undefined' && typeof SiteUtil.getCopyrightYear === 'function'
            ? SiteUtil.getCopyrightYear(footer.copyright_year)
            : (footer.copyright_year || new Date().getFullYear());
        const owner = footer.copyright_owner || 'WWM Emran';
        const links = Array.isArray(footer.links) ? footer.links : [];
        const linkMarkup = links.map(link => {
            const href = escapeHtml(link.url || link.href || '#');
            const label = escapeHtml(link.label || link.name || 'Link');
            return `<a href="${href}">${label}</a>`;
        }).join('<span class="diamond-footer-separator">|</span>');
        container.innerHTML = `
            <div class="diamond-left-footer-logo"><img src="${escapeHtml(logo)}" alt="${escapeHtml(owner)} logo"></div>
            <div class="diamond-left-footer-copy">© ${escapeHtml(copyrightYear)} ${escapeHtml(owner)}</div>
            <div class="diamond-left-footer-links">${linkMarkup}</div>`;
    }
